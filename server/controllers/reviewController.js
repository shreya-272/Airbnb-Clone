...
export const getReviewsForListing = async (req, res) => {
  try {
    ...
    console.error(`[reviewController] Error getting reviews: ${error.message}`);
 ...
 * @access  Public
 */
export const addReviewForListing = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Recalculate average rating and reviewCount on the Listing
    ...
    console.error(`[reviewController] Error adding review: ${error.message}`);
    return res.status(500).json({
      success: false,
